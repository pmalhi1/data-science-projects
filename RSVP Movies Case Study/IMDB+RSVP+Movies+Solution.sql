...
-- Month-wise trend of released movies
...
SELECT 
    ...
    COUNT(rm.movie_id) AS movies_count
FROM
    role_mapping AS rm
        INNER JOIN
...
(
	SELECT  g.genre,
			...
			RANK() OVER (ORDER BY m.worlwide_gross_income DESC) AS movie_rank
	FROM 	
			movie AS m
			INNER JOIN
			genre AS g
			ON m.id = g.movie_id
		WHERE g.genre IN (SELECT g.genre FROM top_3_genre)
)
